...
export default function AssignmentModal({ isOpen, onClose, onSave, editingAssignment }) {
  const [formData, setFormData] = useState({
    subject: '', title: '', instructor: '', instructions: '', details: '',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-white border border-gray-100 rounded-[2rem] w-full max-w-xl max-h-[90vh] flex flex-col shadow-[0_20px_50px_rgba(0,0,0,0.1)] animate-in fade-in zoom-in-95 duration-200 my-auto overflow-hidden">
        {/* Fixed Header */}
        <div className="flex justify-between items-center p-6 border-b border-gray-100 shrink-0 bg-white">
          <h2 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#e581a2] to-[#ffb4c8]">
        ...
        {/* Form wrapping scrollable inputs and fixed footer */}
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden min-h-0">
          ...
          <div className="p-6 space-y-4 overflow-y-auto flex-1">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                ...
                <textarea value={formData.instructions} onChange={e => setFormData({...formData, instructions: e.target.value})} className="w-full bg-[#ececec] border-none rounded-xl p-3 text-gray-900 focus:ring-2 focus:ring-[#e581a2] outline-none min-h-[70px] font-medium" placeholder="e.g. Upload PDF" />
              </div>
              <div className="col-span-2">
                <label className="block text-sm font-bold text-gray-700 mb-1">Assignment Questions / Details</label>
                <textarea value={formData.details} onChange={e => setFormData({...formData, details: e.target.value})} className="w-full bg-[#ececec] border-none rounded-xl p-3 text-gray-900 focus:ring-2 focus:ring-[#e581a2] outline-none min-h-[90px] font-medium" placeholder="Questions or problem description extracted from View Assignment modal" />
              </div>
              <div>
          ...
          {/* Clean Fixed Footer completely separated from scroll content */}
          <div className="p-4 px-6 border-t border-gray-100 flex justify-end gap-3 bg-white shrink-0">
            <button type="button" onClick={onClose} className="px-5 py-2.5 text-sm font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-full transition-colors">
              Cancel
            </button>
            <button type="submit" className="px-5 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-[#e581a2] to-[#ffb4c8] hover:opacity-90 rounded-full transition-opacity shadow-[0_4px_15px_rgba(229,129,162,0.3)]">
              Save Assignment
            </button>
          </div>
        </form>
